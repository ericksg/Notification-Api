...
def create_app() -> FastAPI:
    notifications_app = FastAPI(
        title=settings.PROJECT_NAME, version="1.0.0", lifespan=lifespan
    )

    notifications_app.include_router(
        notifications_router.router, prefix="/notifications", tags=["Notifications"]
    )
    notifications_app.include_router(
        real_time_notifications_router.router,
        prefix="/wsnotifications",
        tags=["WSNotifications"],
    )
